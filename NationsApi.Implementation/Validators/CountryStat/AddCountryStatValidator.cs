...
{
    public class AddCountryStatValidator : AbstractValidator<AddCountryStatDto>
    {
        private NationsContext _context;

        public AddCountryStatValidator(NationsContext context)
        {
            _context = context;
            ...
            RuleFor(x => x.CountryId).NotEmpty()
                .WithMessage("COUNTRY ID is required.")
                    ...
                    .WithMessage("Provided Country doesn't exist").
                        DependentRules(() =>
                        {
                            RuleFor(x => x.CountryId).Must((dto, i) => isCountryYearCombinationUnique(dto))
                            .WithMessage("Combination of COUNTRY ID and YEAR already exists.");
                        });   
                    });
        ...
        private bool isCountryYearCombinationUnique(AddCountryStatDto dto)
        {
            return _context.CountryStats
                .Where(x => x.CountryId == dto.CountryId && x.Year == dto.Year)
                .FirstOrDefault() == null;
        }
    }
}
